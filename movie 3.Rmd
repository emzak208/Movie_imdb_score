...
```
# genre has p=0.02, not significant correlation. therefore, we can drop it.
...
fill <- "Black"
line <- "Red"
ggplot(movie, aes(x = color, y =imdb_score)) +
        geom_boxplot(fill = fill, colour = line) +
        scale_y_continuous(name = "IMDB Score",
                           breaks = seq(0, 11, 0.5),
                           limits=c(0, 11)) +
        scale_x_discrete(name = "Color") +
        ggtitle("Boxplot of IMDB Score and Color")
...
# no genres 
```{r}
movie.sig<-as.data.frame(movie[,c('imdb_score','num_voted_users','num_critic_for_reviews','num_user_for_reviews','duration','facenumber_in_poster','gross','movie_facebook_likes','director_facebook_likes','cast_total_facebook_likes','budget','title_year')])
```
...
#}
#movie.sig.norm <- as.data.frame(lapply(movie.sig, normalize))
```


```{r}
movie.train<-movie.sig[1:2705,]
movie.test<-movie.sig[2706:3005,]
```


Step function to check AIC criteria: 
```{r}
null=lm(movie.train$imdb_score~1) # set null model
summary(null)
```
...
```{r}
full1.initial=lm(movie.train$imdb_score~.^2,data=movie.train)
summary(full1)
```


```{r}
full1<-lm(movie.train$imdb_score~movie.train$num_voted_users+movie.train$num_critic_for_reviews+movie.train$num_user_for_reviews+movie.train$duration+movie.train$facenumber_in_poster+movie.train$gross+movie.train$movie_facebook_likes+movie.train$director_facebook_likes+movie.train$cast_total_facebook_likes+movie.train$budget+movie.train$title_year+movie.train$num_voted_users:movie.train$num_critic_for_reviews+movie.train$num_voted_users:movie.train$duration+movie.train$num_voted_users:movie.train$title_year+movie.train$num_critic_for_reviews:movie.train$num_user_for_reviews+movie.train$duration:movie.train$title_year+movie.train$gross:movie.train$budget+movie.train$cast_total_facebook_likes:movie.train$title_year+movie.train$budget:movie.train$title_year)
summary(full1)
```
...
Choosen:
step 1415
#movie.train$imdb_score ~ movie.train$num_voted_users + movie.train$duration + 
#    movie.train$budget + movie.train$num_critic_for_reviews + 
#    movie.train$title_year + movie.train$num_user_for_reviews + 
#    movie.train$facenumber_in_poster + movie.train$num_voted_users:movie.train$duration + 
#    movie.train$num_voted_users:movie.train$num_critic_for_reviews + 
#    movie.train$duration:movie.train$title_year + movie.train$budget:movie.train$title_year + 
#    movie.train$num_critic_for_reviews:movie.train$num_user_for_reviews + 
#    movie.train$num_voted_users:movie.train$title_year

# After normaliza: (no improvement )
Step:  AIC=-12451.54
#movie.train$imdb_score ~ movie.train$num_voted_users + movie.train$duration + 
#    movie.train$budget + movie.train$num_critic_for_reviews + 
#    movie.train$title_year + movie.train$num_user_for_reviews + 
...
```{r}
# for normalized data
#lm.fit1<-lm(movie.train$imdb_score ~ movie.train$num_voted_users + movie.train$duration + 
#    movie.train$budget + movie.train$num_critic_for_reviews + 
#    movie.train$title_year + movie.train$num_user_for_reviews + 
#    movie.train$facenumber_in_poster + movie.train$num_voted_users:movie.train$duration + 
#    movie.train$num_voted_users:movie.train$num_critic_for_reviews + 
...
```{r}
lm.fit2<- lm(movie.train$imdb_score ~ poly(movie.train$num_voted_users,2) + poly(movie.train$duration,2) + 
    poly(movie.train$budget,2) + poly(movie.train$num_critic_for_reviews,2) + 
    poly(movie.train$title_year,2) + poly(movie.train$num_user_for_reviews,2) + 
    poly(movie.train$facenumber_in_poster,2) + movie.train$num_voted_users:movie.train$duration + 
    movie.train$num_voted_users:movie.train$num_critic_for_reviews + 
    movie.train$duration:movie.train$title_year + movie.train$budget:movie.train$title_year + 
    movie.train$num_critic_for_reviews:movie.train$num_user_for_reviews + 
    movie.train$num_voted_users:movie.train$title_year)
...
```{r}
# on mormalized data : no improvement R^=50.31, points still cluster
# lm.fit2<-lm(movie.train$imdb_score ~ poly(movie.train$num_voted_users,2) + 
# poly(movie.train$duration,2) + 
#    poly(movie.train$budget,2) + poly(movie.train$num_critic_for_reviews,2) + 
#    poly(movie.train$title_year,2) + poly(movie.train$num_user_for_reviews,2) + 
#    poly(movie.train$facenumber_in_poster,2) + 
# movie.train$num_voted_users:movie.train$duration+
#    movie.train$num_voted_users:movie.train$num_critic_for_reviews + 
#    movie.train$duration:movie.train$title_year + movie.train$budget:movie.train$title_year)
# summary(lm.fit2)
```
